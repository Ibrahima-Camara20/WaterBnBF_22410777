...
void wifi_printstatus(int C) {
  ...
  if (C) {
    ...
    s += "\tReceived Sig Strength Indication : " + String(WiFi.RSSI()) +
         " dBm\n";
    s += "\tReceived Sig Strength Indication : " +
...
void wifi_connect_basic(String hostname, String ssid, String passwd) {
  ...
  USE_SERIAL.printf("\nAttempting %d to connect AP of SSID : %s", nbtry,
                    ssid.c_str());
  WiFi.begin(ssid.c_str(), passwd.c_str());
  // WiFi.begin(ssid.c_str(), passwd.c_str(), 0, WiFi.BSSID(thegoodone));
  while (WiFi.status() != WL_CONNECTED && (nbtry < WiFiMaxTry)) {
    delay(SaveDisconnectTime); // 500ms seems to work in most cases, may depend
                               // on AP
    USE_SERIAL.print(".");
    nbtry++;
  }
...
void wifi_connect_multi(String hostname) {
  ...
  // wm.addAP("iPhone", "ibrahima");
  wm.addAP("ougailou", "diallo80");
