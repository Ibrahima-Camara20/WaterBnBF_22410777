...
ESPController::ESPController()
    : luminosity(0), temperature(0.0), coolerState(false), heaterState(false),
      ...
      accessState(STATE_GREEN), accessTimer(0), oneWire(23),
      tempSensor(&oneWire), strip(NUMLEDS, LEDSTRIPPIN, NEO_GRB + NEO_KHZ800) {}
...
void ESPController::update() {
  ...
  // controlSensors(); // Commented out per user request
  managePoolLEDs();
}

void ESPController::setHotspot(bool state) { isHotspot = state; }

void ESPController::setAccess(AccessState state) {
  accessState = state;
  accessTimer = millis();
}
...
void ESPController::progressiveFan() {
  ...
  if (fanSpeed == 0) {
    ...
    if (fanSpeed > 255)
      fanSpeed = 255; // Limiter à 255
    ledcWrite(FANPIN, fanSpeed);
    previousMillisFan = currentMillis;
  } else if (fanSpeed != 0 && fanSpeed != 64 && fanSpeed != 128 &&
             fanSpeed != 192 && fanSpeed != 255) {
    ledcWrite(FANPIN,
              fanSpeed); // Appliquer fanSpeed dynamique si hors progression
  }
}
...
void ESPController::managePoolLEDs() {
  // 1. Manage Pin 2 (Hotspot only)
  if (isHotspot) {
    digitalWrite(2, HIGH);
  } else {
    digitalWrite(2, LOW);
  ...
  // 2. Manage Access State Transitions
  if (accessState == STATE_RED) {
    if (millis() - accessTimer >= 30000) {
      accessState = STATE_GREEN;
    }
  } else if (accessState == STATE_YELLOW) {
    if (millis() - accessTimer >= 3000) {
      if (luminosity < LIGHTMAX) {
        accessState = STATE_GREEN;
      }
    }
  } else{
    if (millis() - accessTimer >= 1000) {
      if (luminosity > LIGHTMAX) {
        accessState = STATE_YELLOW;
      }
    }
  }

  // 3. Control LED Strip based on Access Management
  if (accessState == STATE_RED) {
    setLEDS(strip.Color(255, 0, 0)); // RED for Denied
  } else if (accessState == STATE_YELLOW ) {
    setLEDS(strip.Color(255, 255, 0)); // YELLOW for Occupied
  } else if (accessState == STATE_GREEN) {
    setLEDS(strip.Color(0, 255, 0)); // GREEN for Idle 
  }
}
